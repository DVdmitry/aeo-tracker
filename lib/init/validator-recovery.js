...
export function formatRecoveryPanel({
  ...
  // symptom of mis-inferred category. Promoted from option 3 in 1.0.3: it's
  // a real solution, not an escape hatch.
  lines.push(`    ${c.bold}2.${c.reset} Try again with an explicit category hint (different alternatives may surface):`);
  ...
  // Option 3 — --force escape hatch. Demoted to last in 1.0.3 with a visible
  // yellow warning because it silently pollutes weekly trend data. Kept in
  // the list because it's the documented one-command unblock for users who
