...
export function sectionCompetitorIntelligence(snapshots) {
  ...
  const tableRows = matrix.map(row => {
    const cells = row.cells.map(cell => {
      ...
      if (cell.status === 'yes') {
        ...
        const comps = cell.competitors
          .map(c => `<span style="background:#fff0f0;color:#9f1239;font-size:.75em;padding:2px 7px;border-radius:8px;font-weight:600;border:1px solid #fecdd3;white-space:nowrap">${escMd(c)}</span>`)
          .join(' ');
    ...
    return `<tr><td style="padding:10px 14px;border-bottom:1px solid #f1f5f9;font-size:.85em;color:#334155;font-weight:500;vertical-align:top">${escMd(row.short)}</td>${cells}</tr>`;
  }).join('');
...
const SENTIMENT_BADGE = {
  positive: { fg: '#15803d', bg: '#dcfce7', icon: '👍', label: 'Positive' },
  neutral:  { fg: '#475569', bg: '#f1f5f9', icon: '◌',  label: 'Neutral'  },
  negative: { fg: '#b91c1c', bg: '#fee2e2', icon: '👎', label: 'Negative' },
};
...
export function sectionSentiment(snapshots) {
  ...
  const rows = withSentiment.map(r => {
    ...
    const conf = r.sentiment.confidence === 'high' ? ''
      : ` <span style="color:#94a3b8;font-size:.85em">(${escMd(r.sentiment.confidence)})</span>`;
    const badge = `<span style="background:${b.bg};color:${b.fg};padding:2px 8px;border-radius:10px;font-size:.85em;font-weight:600;white-space:nowrap">${b.icon} ${b.label}</span>${conf}`;
...
export function sectionDomainShareOfVoice(snapshots) {
  ...
  const rows = top.map(d => {
    ...
    const bar = `<span style="display:inline-block;width:${barWidth}%;max-width:140px;height:8px;background:linear-gradient(90deg,#4f46e5,#7c3aed);border-radius:4px;vertical-align:middle"></span>`;
...
export function sectionHistoricalTrend(snapshots) {
  ...
  const spark = sparkline({ values, width: 480, height: 80 });
  const first = present[0];
  const last = present[present.length - 1];
  const delta = last - first;
  const arrow = delta > 0 ? `↑ +${delta}` : delta < 0 ? `↓ ${delta}` : '→ flat';
  const trendColor = delta > 0 ? '#15803d' : delta < 0 ? '#b91c1c' : '#475569';

  const dateRange = `${recent[0].date} → ${recent[recent.length - 1].date}`;
  const tickRow = recent
    .map(s => `<td style="text-align:center;font-size:.75em;color:#64748b;padding:4px 6px">${(s.date || '').slice(5)}<br/><strong style="color:#0f172a">${s.score ?? '—'}%</strong></td>`)
    .join('');

  return `## Visibility Trend — last ${recent.length} run${recent.length !== 1 ? 's' : ''}

_${dateRange} · <span style="color:${trendColor};font-weight:600">${arrow} pts</span> overall. Each tick = one weekly run._

<div style="margin:16px 0;padding:16px;background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0">
${spark}
<table style="width:100%;border-collapse:collapse;margin-top:8px"><tr>${tickRow}</tr></table>
</div>
`;
}
...
export function sectionOutreachTemplates(snapshots) {
  ...
  const blocks = templates.map((t, i) => {
    ...
    const why = t.why ? `<div style="margin-top:8px;padding:8px 12px;background:#fef9c3;color:#854d0e;border-radius:8px;font-size:.85em"><strong>Why this domain:</strong> ${escMd(t.why)}</div>` : '';
    return `<details${i === 0 ? ' open' : ''} style="margin:12px 0;padding:12px 16px;background:#fff;border:1px solid #e2e8f0;border-radius:10px">
<summary style="cursor:pointer;font-weight:600;font-size:1.05em;color:#0f172a">${i + 1}. ${escMd(t.host)}</summary>
<div style="margin-top:12px"><strong style="color:#475569;font-size:.85em">Subject:</strong> ${subject}</div>
<div style="margin-top:8px;padding:12px;background:#f8fafc;border-radius:8px;font-family:Georgia,serif;color:#1e293b;line-height:1.6">${body}</div>
${why}
</details>`;
  }).join('\n');
...
function radarStatsForBrand(latest, brandName) {
      ...
      if (isUserBrand) {
        if (r.mention === 'yes' || r.mention === 'src') {
          ...
          if (typeof r.position === 'number' && r.position > 0) {
            rankSum += Math.max(0, 100 - (r.position - 1) * 15);
            rankN++;
          }
        }
      } else {
  ...
  const sentiment = sentimentN > 0 ? sentimentSum / sentimentN : 50;
  const rank = rankN > 0 ? rankSum / rankN : (mentionTotal > 0 ? 50 : 0);
  const mentionScore = Math.min(100, mentionTotal * 20);
...
export function sectionCompetitorRadar(snapshots) {
  ...
  const buildRadar = (s, isUser) => {
    ...
    const accent = isUser ? '#4f46e5' : '#94a3b8';
    const bgTone = isUser ? '#eef2ff' : '#f8fafc';
    return `<div style="text-align:center;padding:12px;background:${bgTone};border-radius:12px;border:1px solid #e2e8f0">
<div style="font-weight:700;color:#0f172a;margin-bottom:4px">${isUser ? '★ ' : ''}${escMd(s.name)}</div>
<div style="font-size:.8em;color:#64748b;margin-bottom:8px">${s.rawMentions} mention${s.rawMentions !== 1 ? 's' : ''}</div>
${radar({ axes, size: 220 }).replace('fill="#f59e0b" fill-opacity="0.18" stroke="#f59e0b"', `fill="${accent}" fill-opacity="0.18" stroke="${accent}"`)}
</div>`;
  };
...
const ACCESS_BADGE = {
  allowed:     { fg: '#15803d', bg: '#dcfce7', icon: '✓', label: 'Allowed' },
  blocked:     { fg: '#b91c1c', bg: '#fee2e2', icon: '✗', label: 'Blocked' },
  partial:     { fg: '#854d0e', bg: '#fef9c3', icon: '◐', label: 'Partial' },
  unspecified: { fg: '#475569', bg: '#f1f5f9', icon: '—', label: 'Unspecified' },
};
...
export function sectionCrawlability(snapshots) {
  ...
  const fileCheck = (label, found, extra = '') => {
    const icon = found ? '✅' : '❌';
    const tone = found ? '#15803d' : '#b91c1c';
    return `<span style="color:${tone};font-weight:600">${icon} ${label}</span>${extra ? ` <span style="color:#64748b;font-size:.85em">${extra}</span>` : ''}`;
  };
  ...
  const rows = Object.entries(byProvider).map(([provider, bots]) => {
    const cells = bots.map(b => {
      ...
      return `<span style="background:${a.bg};color:${a.fg};padding:3px 9px;border-radius:8px;font-size:.85em;font-weight:600;white-space:nowrap;margin:2px 4px 2px 0;display:inline-block">${a.icon} ${b.label}</span>`;
    }).join('');
...
export function sectionDomainCategories(snapshots) {
  ...
  const rows = categories.map(cat => {
    ...
    const more = cat.domains.length > 3 ? ` <span style="color:#94a3b8">+${cat.domains.length - 3} more</span>` : '';
...
export function sectionFunnelBreakdown(snapshots) {
  ...
  const rows = sorted.map(t => {
    ...
    const tone = rate >= 60 ? '#15803d' : rate >= 25 ? '#854d0e' : '#b91c1c';
    const verdict = rate >= 60 ? 'strong' : rate >= 25 ? 'present' : rate > 0 ? 'emerging' : 'invisible';
    const barW = Math.max(2, rate);
    const bar = `<span style="display:inline-block;width:${barW}%;max-width:140px;height:8px;background:${tone};border-radius:4px;vertical-align:middle"></span>`;
    return `| **${escMd(t.tag)}** | ${t.mentions}/${t.total} | <span style="color:${tone};font-weight:700">${rate}%</span> ${verdict} | ${bar} |`;
  }).join('\n');
...
export function sectionActionableGaps(snapshots, opts = {}) {
  ...
  const rows = prioritised.map(({ r, comps }) => {
    ...
    const compsBadge = comps.slice(0, 3).map(c => `<span style="background:#fee2e2;color:#9f1239;padding:1px 7px;border-radius:8px;font-size:.8em;margin-right:3px">${escMd(c)}</span>`).join('');
...
export function sectionGeoComparison(snapshots) {
  ...
  const headerCells = providers.map(p => `<th style="padding:8px 12px;background:#4f46e5;color:#fff;font-size:.85em;text-align:center">${providerLabel(p)}</th>`).join('');
  ...
  const rows = regions.map(region => {
    ...
    const cells = providers.map(p => {
      ...
      if (!stat) return `<td style="padding:8px 12px;text-align:center;color:#94a3b8">—</td>`;
      const tone = stat.rate >= 60 ? '#15803d' : stat.rate >= 25 ? '#854d0e' : stat.rate > 0 ? '#b91c1c' : '#94a3b8';
      const bg   = stat.rate >= 60 ? '#dcfce7' : stat.rate >= 25 ? '#fef9c3' : stat.rate > 0 ? '#fee2e2' : '#f1f5f9';
      return `<td style="padding:8px 12px;text-align:center;background:${bg};color:${tone};font-weight:700">${stat.rate}%<br/><span style="font-size:.75em;font-weight:400">${stat.hits}/${stat.total}</span></td>`;
    }).join('');
    return `<tr><td style="padding:8px 12px;font-weight:600;color:#0f172a">${label}</td>${cells}</tr>`;
  }).join('');
...
<div style="overflow-x:auto;margin:16px 0"><table style="width:100%;border-collapse:collapse;border-radius:10px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.06)"><thead><tr><th style="padding:8px 12px;background:#4f46e5;color:#fff;text-align:left">Region</th>${headerCells}</tr></thead><tbody>${rows}</tbody></table></div>
...
export function sectionUnifiedVisibilityIndex(snapshots) {
  ...
  const tone = uvi >= 70 ? '#15803d' : uvi >= 40 ? '#854d0e' : uvi > 0 ? '#b91c1c' : '#94a3b8';
  const verdict = uvi >= 70 ? 'STRONG' : uvi >= 40 ? 'PRESENT' : uvi > 0 ? 'EMERGING' : 'INVISIBLE';

  const componentRow = (label, value, weight, hint) => {
    const t = value >= 60 ? '#15803d' : value >= 25 ? '#854d0e' : value > 0 ? '#b91c1c' : '#94a3b8';
    const barW = Math.max(2, value);
    return `| **${label}** | <span style="color:${t};font-weight:700">${value}/100</span> | ${(weight * 100).toFixed(0)}% | <span style="display:inline-block;width:${barW}%;max-width:140px;height:8px;background:${t};border-radius:4px;vertical-align:middle"></span> | ${hint} |`;
  };
...
<div style="text-align:center;padding:20px;background:#f8fafc;border-radius:14px;border:1px solid #e2e8f0;margin:16px 0">
<div style="font-size:.85em;color:#64748b;letter-spacing:.06em;text-transform:uppercase">Unified Visibility Index</div>
<div style="font-size:64px;font-weight:800;color:${tone};line-height:1;margin:8px 0">${uvi}<span style="font-size:24px;color:#94a3b8;font-weight:600"> / 100</span></div>
<div style="font-size:1em;font-weight:700;color:${tone};letter-spacing:.05em">${verdict}</div>
<div style="font-size:.85em;color:#64748b;margin-top:6px">Composite of 4 signals · sample size ${c.sample} cell${c.sample !== 1 ? 's' : ''}</div>
</div>
...
export function sectionDiscoverability(snapshots) {
  ...
  const tone = result.score >= 70 ? '#15803d' : result.score >= 40 ? '#854d0e' : '#b91c1c';
  const rows = Object.entries(result.breakdown).map(([key, b]) => {
    const t = b.value >= 60 ? '#15803d' : b.value >= 25 ? '#854d0e' : b.value > 0 ? '#b91c1c' : '#b91c1c';
    return `| **${key}** | <span style="color:${t};font-weight:700">${b.value}/100</span> | ${(b.weight * 100).toFixed(0)}% | ${b.note} |`;
  }).join('\n');

  return `## Discoverability Score

<div style="display:flex;gap:20px;align-items:center;padding:16px 20px;background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;margin:16px 0">
<div style="font-size:48px;font-weight:800;color:${tone};line-height:1">${result.score}<span style="font-size:18px;color:#94a3b8;font-weight:600">/100</span></div>
<div style="flex:1">
<div style="font-weight:700;color:#0f172a">Site readiness for AI crawlers</div>
<div style="font-size:.9em;color:#64748b;margin-top:4px">Derived from robots.txt, AI bot access matrix, sitemap.xml, and /llms.txt presence — no extra HTTP requests beyond the crawlability audit.</div>
</div>
</div>
...
export function sectionTopicClusters(snapshots) {
  ...
  const rows = clusters.map(cl => {
    const t = cl.rate >= 60 ? '#15803d' : cl.rate >= 25 ? '#854d0e' : cl.rate > 0 ? '#b91c1c' : '#94a3b8';
    const exampleQueries = cl.queries.slice(0, 3).map(q => `<code>${escMd(q.text.slice(0, 50)).replace(/\|/g, '\\|')}${q.text.length > 50 ? '…' : ''}</code>`).join(' · ');
    return `| **${escMd(cl.topic)}** | ${cl.queries.length} | <span style="color:${t};font-weight:700">${cl.rate}%</span> (${cl.hits}/${cl.total}) | ${exampleQueries} |`;
  }).join('\n');
...
export function sectionAuthorityPresence(snapshots) {
  ...
  const wikiBadge = ap.wikipedia.found
    ? (ap.wikipedia.isDisambiguation
        ? `<span style="background:#fef9c3;color:#854d0e;padding:3px 10px;border-radius:8px;font-weight:700">⚠ Disambiguation page</span>`
        : `<span style="background:#dcfce7;color:#15803d;padding:3px 10px;border-radius:8px;font-weight:700">✓ Article exists</span>`)
    : `<span style="background:#fee2e2;color:#b91c1c;padding:3px 10px;border-radius:8px;font-weight:700">✗ No article</span>`;

  // Wikipedia/Reddit content is third-party-controlled — escape before HTML.
  const wikiUrl = ap.wikipedia.found
    ? `[View on Wikipedia](${ap.wikipedia.pageUrl})`
    : `[Create one](${ap.wikipedia.queryUrl || 'https://en.wikipedia.org/wiki/Wikipedia:Your_first_article'})`;
  const wikiExtract = ap.wikipedia.found && ap.wikipedia.extract
    ? `<br/><span style="color:#64748b;font-size:.9em">"${escMd(ap.wikipedia.extract)}…"</span>`
    : '';

  const redBadge = ap.reddit.found
    ? `<span style="background:#dcfce7;color:#15803d;padding:3px 10px;border-radius:8px;font-weight:700">✓ ${ap.reddit.mentionCount}${ap.reddit.capped ? '+' : ''} posts</span>`
    : `<span style="background:#fee2e2;color:#b91c1c;padding:3px 10px;border-radius:8px;font-weight:700">✗ Not discussed</span>`;

  const subRows = (ap.reddit.topSubs || []).map(s => `<code>r/${escMd(s.name)}</code> (${s.count})`).join(' · ');

  // Tips wrapped in a collapsible <details> so the cell stays compact
  // (~280px tall, matching the Site Readiness card on its left in the bento
  // grid). Inlining tips inflated the cell to ~480px and broke balance.
  let advisory = '';
  const tips = [];
  if (!ap.wikipedia.found) tips.push('No Wikipedia article — earn coverage in 3+ independent reliable sources first, then a third party can create one (you cannot create your own per WP:COI).');
  if (ap.wikipedia.isDisambiguation) tips.push('Wikipedia entry is a disambiguation — your brand competes for the term. Earn enough notability to claim the primary topic.');
  if (!ap.reddit.found) tips.push('No Reddit discussion — find subreddits in your category (search bar) and answer questions with verifiable expertise. Do not spam.');
  if (tips.length > 0) {
    const tipItems = tips.map(t => `<li>💡 ${t}</li>`).join('');
    advisory = `\n<details><summary>Why this matters · ${tips.length} hint${tips.length !== 1 ? 's' : ''}</summary><ul style="margin:8px 0 0;padding:0 0 0 18px;font-size:12.5px;line-height:1.5;color:var(--ink-2)">${tipItems}</ul></details>\n`;
  }

  return `## Authority-Source Presence

_Wikipedia and Reddit are two off-page signals AI engines weight heavily — they are part of the ground-truth corpus most LLMs trained on._

| Source | Status | Detail |
|---|---|---|
| **Wikipedia** | ${wikiBadge} | ${wikiUrl}${wikiExtract} |
| **Reddit** | ${redBadge} | ${subRows || '—'} |
${advisory}`;
}
...
`;
  ...
  const sampleBlocks = (ads.samples || []).slice(0, 5).map(s => {
    ...
    return `<div style="margin:8px 0;padding:10px 14px;background:#fef9c3;border-left:3px solid ${tone};border-radius:6px"><div style="font-size:.85em;color:${tone};font-weight:700">${providerLabel(s.provider)} · ${escMd(s.query)} · <code>${escMd(s.kind)}</code></div><div style="margin-top:4px;color:#1e293b;font-style:italic">"${snip}"</div></div>`;
  }).join('');
