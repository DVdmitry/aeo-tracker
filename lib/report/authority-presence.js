...
export async function checkWikipedia(brand, { fetchImpl = fetchWithTimeout } = {}) {
  ...
  try {
    ...
    if (res.status === 404) {
      // Wikipedia's /wiki/ URL also 404s for missing articles instead of
      // redirecting to a create flow. Point users at the edit URL so the
      // "Create one" link opens the article-editor UI instead of a dead page.
      return { found: false, status: 404, url, queryUrl: `https://en.wikipedia.org/w/index.php?title=${slug}&action=edit` };
    }
    if (!res.ok) {
