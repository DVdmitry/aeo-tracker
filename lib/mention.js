...
export function findPosition(text, brand, domain) {
  ...
  for (const line of lines) {
    ...
    // +1 for the consumed '\n'. Assumes LF: providers' responses arrive over
    // HTTP / SDK with newline normalisation, so CRLF in `text` would only
    // happen if a caller paste-loads from a Windows file — acceptable risk.
    cumOffset = lineEnd + 1;
  }
  return null;
}
